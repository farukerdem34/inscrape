    ...
    def getFollowers(self, username: str, verbose: bool = False):
        ...
        while True:
            ...
            if not (follower_count == len(followersList)):
                follower_count = len(followersList)
                if verbose:
                    self.verbose_list(q.find_element(By.CSS_SELECTOR,"a").get_attribute("href")[26:-1] for q in followersList)
            else:
                break
    ...
    def getFollowing(self, username: str, verbose: bool = False):
        ...
        while True:
            ...
            if not (followingsCount == len(followingsList)):
                followingsCount = len(followingsList)
                if verbose:
                    self.verbose_list(q.find_element(By.CSS_SELECTOR,"a").get_attribute("href")[26:-1] for q in followingsList)
            else:
                break
...
instagram = Instagram(signin_method=signin_method,
